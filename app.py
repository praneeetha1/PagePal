...
def init_session():
    ...
    if "embedding_generator" not in st.session_state:
        ...
        index_name = f"pagepal-{st.session_state.user_id}"
        ...
        with st.spinner("Processing documents..."):
            try:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    # Store with user-specific index
        ...
        if prompt := st.chat_input("Ask about your documents:"):
            ...
            with st.spinner("Analyzing..."):
                ...
                results = st.session_state.embedding_generator.query_embeddings(
                    query=prompt,
                    top_k=3,
                    filter={"user_id": st.session_state.user_id}  # User-specific filter
                )
                
                context = "\n".join([
                    f"Document excerpt: {match['metadata'].get('text','')}"
